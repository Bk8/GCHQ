...
#include "littleendian_binary_archive.hpp"

class littleendian_binary_iarchive {
  std::istream & m_is;
  ...
  void load(T &t){
	  ...
	  BOOST_DEDUCED_TYPENAME boost::mpl::eval_if<boost::is_enum< T >,
						     boost::mpl::identity<load_enum_type<littleendian_binary_iarchive> >,
						     //else
							 ...
							 >,
						       boost::mpl::identity<load_primitive<littleendian_binary_iarchive> >,
						       // else
						       boost::mpl::identity<load_only<littleendian_binary_iarchive> >
						       > >::type typex;
	typex::invoke(*this, t);
  }    
public:
  ...
  // the << operators 
  template<class T>
  littleendian_binary_iarchive & operator>>(T & t){
    load(t);
    return * this;
  }

  template<class T, int N>
  littleendian_binary_iarchive & operator>>(T (&t)[N]){
  ...
  template<class T>
  littleendian_binary_iarchive & operator>>(boost::serialization::nvp< T > & t){
    * this >> t.value();
    return * this;
  }
  ...
  littleendian_binary_iarchive(std::istream & is) :
    m_is(is) {}
};
